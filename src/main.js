...
import { fromTemplate, templateOf } from "./templates.js";
import { FINISH_KINDS, KIND_COLORS, SURFACE_KINDS, ceilingFinish, floorFinish, wallFinish } from "./finishes.js";
...
async function boot() {
  ...
  let p,
    selected = null,
    // The room open in the Rooms tab, by id. View state, like `selected`.
    selectedRoom = null,
    // What "Add a room beside it" adds next. View state.
    newRoomType = "bedroom",
...
<div class="workspace"><aside class="library" id="library"></aside><main class="editor"><div class="toolbar"><div class="toolgroup">${btn("select", "Select", "mouse-pointer-2", "active")}${btn("move", "Move", "move")}${btn("snap", "Snap 1″", "grid-2x2", "active")}${btn("measure", "Measure", "ruler")}${btn("walk", "Walk", "footprints")}${btn("draw-box", "Box", "square")}${btn("draft", "Fast edit", "zap")}${btn("draft-lock", "Fast edit: follows the gesture", "lock", "draft-lock")}</div><div class="toolgroup">${btn("undo", "Undo", "undo-2", "icon-only")}${btn("redo", "Redo", "redo-2", "icon-only")}</div><div class="mode-switch"><button data-action="mode-3d">3D home</button><button data-action="mode-photo">Photo</button></div>${btn("export-tab", "Export", "download", "export-top")}</div><div class="viewport"><div id="scene"></div><div id="photo" hidden></div><div class="scene-label"><span class="eyebrow" id="mode-label">MEASURED WORKSPACE</span><strong id="scene-title"></strong><span id="scene-subtitle"></span></div><div id="photo-empty" hidden><div>${icon("image-plus")}<h2>Start with a photo of your room</h2><p>Add artwork and adjust its four corners to match the wall perspective.</p>${btn("upload-photo", "Upload room photo", "plus", "primary")}</div></div><div class="walk-pad" hidden><button data-walk="forward" aria-label="Step forward">▲</button><button data-walk="left" aria-label="Step left">◀</button><button data-walk="back" aria-label="Step back">▼</button><button data-walk="right" aria-label="Step right">▶</button><button data-action="walk" class="walk-exit" aria-label="Stop walking">Done</button></div><div class="viewport-bottom"><div class="view-switch" id="view-switch"><button data-view="perspective" class="active">Perspective</button><button data-view="back">Back</button><button data-view="left">Left</button><button data-view="right">Right</button><button data-view="plan">Plan</button></div><label class="saved-view-pick" hidden><span>View</span><select id="saved-view" aria-label="Go to a saved view"></select></label><div class="zoom-controls"><span class="zoom-label">Zoom</span>${btn("zoom-out", "Zoom out", "minus", "icon-only")}${btn("zoom-in", "Zoom in", "plus", "icon-only")}${btn("reset-view", "Reset view", "rotate-ccw", "icon-only")}</div></div></div><div class="statusbar"><span id="gesture-hint">Drag to orbit · scroll or +/− to zoom · right-drag to pan</span><label class="preview-quality" title="Preview quality: how many pixels the viewport draws for each one on screen. Exports are never affected."><span>Preview</span><select id="quality-quick" aria-label="Preview quality"></select><output id="quality-now"></output></label><span id="selection-status"></span></div></main><aside class="inspector"><button class="sheet-toggle" data-action="sheet-toggle" aria-label="Fold the panel away" title="Fold the panel away"><span>Fold</span></button><div class="inspector-tabs">${["art", "rooms", "layout", "walls", "lighting", "video", "export"].map((t, i) => `<button data-tab="${t}">${icon(["image", "house", "layout-panel-left", "columns-2", "lightbulb", "video", "download"][i])}<span>${["Artwork", "Rooms", "Layout", "Walls", "Lighting", "Video", "Export"][i]}</span></button>`).join("")}</div><div id="inspector-content"></div></aside></div><footer><span class="footer-brand">${icon("house")} HOME LAYOUT <small>Preview</small><small id="build-stamp" title="Version ${BUILD.version} · built ${BUILD.time} · commit ${BUILD.commit}">v${BUILD.version} · ${BUILD.short} UTC · ${BUILD.commit}</small></span><span>Your images. Your space. Your arrangement.</span><span id="network">Local workspace</span></footer><input type="file" id="art-input" accept="image/jpeg,image/png" multiple hidden/><input type="file" id="replace-input" accept="image/jpeg,image/png" hidden/><input type="file" id="photo-input" accept="image/jpeg,image/png" hidden/><input type="file" id="surround-input" accept="image/jpeg,image/png" hidden/><input type="file" id="ground-input" accept="image/jpeg,image/png" hidden/><input type="file" id="underlay-input" accept="image/jpeg,image/png" hidden/><input type="file" id="model-input" accept=".glb,model/gltf-binary" hidden/><input type="file" id="backup-input" accept=".json,.booth" hidden/><div id="toast" role="status"></div><dialog id="dialog"><div id="dialog-content"></div></dialog><dialog id="image-editor"><div id="image-editor-content"></div></dialog><dialog id="timeline-dialog" class="timeline-dialog"><div id="timeline-content"></div></dialog>`;
  ...
  if (scene) {
    ...
    scene.onDrawBox = (r) => {
      if (!allowed("box")) return toast("Draw a box is part of Home Layout Pro.", true);
      const list = boothPedestals(p);
  ...
  function proLock(feature, what = "") {
    ...
    return `<section class="pro-lock" data-pro-lock="${e(feature)}"><h3>${e(name)} <span class="pro-badge">${icon("lock")}Pro</span></h3><p class="muted">${e(what || name + " is part of Home Layout Pro.")}</p>${btn("tier-pro", "Switch to Pro", "zap", "wide")}</section>`;
  }
  ...
  function setTier(next) {
    ...
    renderInspector();
    syncClearance();
    toast(tier === "pro" ? "Pro: every tool is unlocked." : "Lite: Pro tools show a lock. Anything already in this home still draws.");
  }
  function panelFields() {
    const panels = boothPanels(p);
    return `<section><h3>Free-standing walls</h3><p class="muted">Free-standing walls you can stand anywhere in the house and hang art on either side. Position is measured in inches from the centre of the floor: X is east, Z is south. They do not change the rooms.</p><p class="muted">Click a free-standing wall in the viewport to select it, then drag it across the floor or use the sliders. Snap keeps a drag on whole inches.</p>${panels.map((panel, i) => {
  ...
  function pedestalFields() {
    const list = boothPedestals(p);
    return `<section><h3>Furniture</h3><p class="muted">Beds, sofas, tables, kitchen counters, cabinets, shelves, a rug, a TV and the bathroom, each at its usual size and all of them resizable. A new piece goes into the room open in the Rooms tab, against its north wall if it is the kind that stands against one. Double-click one in the viewport to pick it up, then drag it across the floor or use the sliders; R turns it. Position is inches from the centre of the floor: X is east, Z is south. Plan view shows the clear floor around the one selected.</p>${list.map((ped, i) => {
      const name = ped.name || "Pedestal " + (i + 1),
        scope = "pedestal-" + ped.id,
        chosen = selectedPedestal === ped.id,
        f = (label, key, value, min, max, step, unit) =>
          field(label, key, value, min, max, step, unit, scope, name + " " + label);
      return `<div class="wall-setting${chosen ? " selected" : ""}${isShown(ped) ? "" : " is-hidden"}" data-pedestal="${e(ped.id)}"><div class="panel-heading"><h4>${e(name)}${chosen ? ' <span class="badge">Selected</span>' : ""}${isShown(ped) ? "" : ' <span class="badge">Hidden</span>'}</h4><span class="piece-actions">${hideEye("pedestal", ped.id, name, isShown(ped))}${btn("delete-pedestal-" + ped.id, "Remove " + name, "trash-2", "icon-only")}</span></div>${furnitureKind(ped) === "box" ? `${f("Height", "height", ped.height, ...BOX_LIMITS.height, 1, "in")}${range("Pull up", "height", ped.height, BOX_LIMITS.height[0], Math.max(96, Math.ceil(ped.height)), 1, scope, "in")}${f("Width", "width", ped.width, ...BOX_LIMITS.width, 1, "in")}${f("Depth", "depth", ped.depth, ...BOX_LIMITS.depth, 1, "in")}` : `${f("Height", "height", ped.height, ...furnitureLimits(furnitureKind(ped)).height, 1, "in")}${f("Width", "width", ped.width, ...furnitureLimits(furnitureKind(ped)).width, 1, "in")}${f("Depth", "depth", ped.depth, ...furnitureLimits(furnitureKind(ped)).depth, 1, "in")}`}${colorField("Finish", "color", scope, ped.color || PEDESTAL.color, name + " finish")}${f("Position X", "x", ped.x, -600, 600, 1, "in")}${pedestalSlider(ped, name, "x", "Slide left / right")}${f("Position Z", "z", ped.z, -600, 600, 1, "in")}${pedestalSlider(ped, name, "z", "Slide front / back")}${f("Rotation", "rotation", ped.rotation, -180, 180, 5, "°")}</div>`;
    }).join("")}${list.length < MAX_PEDESTALS ? `<label class="setting-label">Add to the floor<select id="furniture-kind" aria-label="Furniture to add">${furnitureOptions()}</select></label>${btn("add-pedestal", "Add " + FURNITURE[furnitureChoice].label.toLowerCase(), "plus", "wide")}` : `<p class="muted">${MAX_PEDESTALS} pieces is the limit.</p>`}</section>`;
  }
  /**
   * The furniture picker's options, grouped by the room each piece usually
   * stands in. The booth app's pieces are offered only on the booth fixture,
   * which has no rooms; a home still draws any it was saved with.
   */
  function furnitureOptions() {
    const home = homeRooms(p).length > 0;
    const offered = Object.entries(FURNITURE).filter(([k, f]) => (k !== "box" || allowed("box")) && !(home && f.booth));
    const option = ([k, f]) => `<option value="${k}" ${k === furnitureChoice ? "selected" : ""}>${e(f.label)} · ${f.width}×${f.depth}″</option>`;
    const groups = [...FURNITURE_ROOMS, undefined].map((room) => [room, offered.filter(([, f]) => f.room === room)]).filter(([, list]) => list.length);
    return groups.map(([room, list]) => (room ? `<optgroup label="${e(room)}">${list.map(option).join("")}</optgroup>` : list.map(option).join(""))).join("");
  }
  /**
...
.filter(([id, asset]) => !excluded.has(id) && (asset.role === "artwork" || p.art.some((item) => item.asset === id)))
  ...
  function placementFromTemplate(template, keepEdits = false) {
    const current = p.art.find((item) => item.id === selected && !item.booth);
  ...
  function signFields(a) {
    ...
    return `<section><h3>Details</h3>${input("Medium","medium",a.medium)}${input("Price","price",a.price)}<p class="muted">Kept with this work in the project and its backups.</p></section>`;
  }
  ...
  function personSlider(person, key, label, unit = "in") {
    ...
    // outside the house looking in is half of what these figures are for, so
    ...
    // outside the house is a mistake rather than a photograph.
  ...
  function viewsSection() {
    ...
    const rows = views
      ...
      .join("");
    return `<section><h3>Saved views <span>${views.length} / ${MAX_VIEWS}</span></h3><p class="muted">Frame the house the way you want it seen, then save the view. Each one is a click away here and in the View menu under the viewport, and they are saved with the home.</p>${rows}${views.length < MAX_VIEWS ? btn("view-save", "Save this view", "bookmark", "wide") : `<p class="muted">${MAX_VIEWS} views is the limit.</p>`}${views.length ? btn("views-export", `Export all ${views.length} as PNG`, "download", "wide") : ""}</section>`;
  }
  /**
  ...
  function tagsSection() {
    return `<section><h3>Tags <span>show or hide</span></h3><p class="muted">Hide a whole group at once — in the viewport and in exports. Nothing is deleted, and reopening the home shows everything again.</p><div class="tag-list">${Object.entries(TAGS)
   ...
   * maker's own scan — a sculpture, a piece of furniture you own — stood on the floor at
  ...
  function modelsSection() {
    ...
    const rows = models
      ...
      .join("");
    return gated("glb", `<section><h3>3D models <span>${models.length} / ${MAX_MODELS}</span></h3><p class="muted">Bring in a .glb from SketchUp, Blender or a 3D scan — a sculpture, a piece of furniture you own — and stand it on the floor. It is scaled to the height you type, so its own units do not matter. Saved in the home and its backups.</p>${rows}${models.length < MAX_MODELS ? btn("upload-model", "Import .glb model", "upload", "wide") : `<p class="muted">${MAX_MODELS} models is the limit.</p>`}</section>`, "Bring in .glb models from SketchUp, Blender or a scan, and export the home as .glb. Part of Home Layout Pro.");
  }
  /**
  ...
  function clearanceSection() {
    ...
    const rows = issues
      ...
      .join("");
    return gated("clearance", `<section><h3>Clearance checks <span>${issues.length ? issues.length + " to look at" : "all clear"}</span></h3><p class="muted">Floor pieces standing in each other or outside the house, works hung over each other, and gaps narrower than the ${ACCESSIBLE}″ a wheelchair needs. Plan view draws the tight gaps in red. Pieces pushed right up against something are taken as meant.</p>${issues.length ? `<ul class="clearance-list">${rows}</ul>` : `<p class="ok-note">Nothing is in the way.</p>`}</section>`, "Warnings for pieces standing in each other or outside the house, and gaps too narrow for a wheelchair. Part of Home Layout Pro.");
  }
  ...
  /**
   * Rooms: the house itself. A list of rooms, the one open for editing —
   * its size and place in inches, which of its four walls stand, and its
   * doors, windows and archways — and buttons to add a room against any side
   * of it, so rooms meet on one line and share the wall between them.
   */
  function roomsHTML() {
    const rooms = homeRooms(p);
    if (selectedRoom && !findRoom(p, selectedRoom)) selectedRoom = null;
    ...
    const openings = (r.openings || []).map((o) => {
      const os = `opening|${r.id}|${o.id}`,
        len = roomEdge(r, o.side).length;
      const of = (label, key, lo, hi) => field(label, key, o[key] ?? 0, lo, hi, 1, "in", os, `${OPENING_KINDS[o.kind].label} ${label}`);
      return `<div class="opening-row"><h4>${OPENING_KINDS[o.kind].label} · ${SIDE_NAMES[o.side]} wall</h4><label class="setting-label">Wall<select data-field="side" data-scope="${os}" aria-label="${OPENING_KINDS[o.kind].label} wall">${SIDES.map((sd) => `<option value="${sd}" ${o.side === sd ? "selected" : ""}>${SIDE_NAMES[sd]}</option>`).join("")}</select></label>${of("From left end", "x", 0, Math.max(0, len - o.width))}<div class="field-pair">${of("Width", "width", OPENING_LIMITS.width[0], Math.min(OPENING_LIMITS.width[1], len))}${of("Height", "height", OPENING_LIMITS.height[0], r.height)}</div>${o.kind === "window" ? of("Sill height", "sill", 0, r.height - 12) : ""}${o.kind === "door" ? `<div class="field-pair"><label class="setting-label">Hinge<select data-field="hinge" data-scope="${os}" aria-label="Door hinge"><option value="left" ${o.hinge !== "right" ? "selected" : ""}>Left</option><option value="right" ${o.hinge === "right" ? "selected" : ""}>Right</option></select></label><label class="setting-label">Opens<select data-field="swing" data-scope="${os}" aria-label="Door opens"><option value="in" ${o.swing !== "out" ? "selected" : ""}>Into this room</option><option value="out" ${o.swing === "out" ? "selected" : ""}>Away from it</option></select></label></div>${range("Door open", "angle", o.angle ?? 70, 0, 180, 1, os, "°")}` : ""}<button data-action="opening-delete" data-room="${r.id}" data-opening="${o.id}" class="text-button">${icon("trash-2")}<span>Remove ${OPENING_KINDS[o.kind].label.toLowerCase()}</span></button></div>`;
    }).join("");
    const addOpening = (r.openings || []).length < MAX_OPENINGS
      ? `<div class="button-row">${Object.entries(OPENING_KINDS).map(([k, v]) => `<button data-action="opening-add" data-room="${r.id}" data-kind="${k}">${icon("plus")}<span>${v.label}</span></button>`).join("")}</div><p class="muted">Added to the south wall; choose its wall and place it below. Positions run from the wall's left end, seen from inside this room.</p>`
      : `<p class="muted">${MAX_OPENINGS} openings is the most one room holds.</p>`;
    ...
    return `${heading}${list}<section><h3>${e(r.name)}</h3><label class="setting-label">Name<input type="text" data-field="name" data-scope="${scope}" maxlength="120" aria-label="Room name" value="${e(r.name)}"/></label><label class="setting-label">Kind of room<select data-field="type" data-scope="${scope}" aria-label="Kind of room">${Object.entries(ROOM_TYPES).map(([k, v]) => `<option value="${k}" ${(r.type || "other") === k ? "selected" : ""}>${v}</option>`).join("")}</select></label><div class="field-pair">${f("Width", "width")}${f("Depth", "depth")}</div>${f("Ceiling height", "height")}<div class="field-pair">${f("Centre X", "x")}${f("Centre Z", "z")}</div><p class="muted">Width runs east–west, depth north–south. X and Z are the room's centre from the middle of the floor; +Z is south.</p></section><section><h3>Walls</h3>${walls}<p class="muted">Untick a wall to open the room up — open plan to the next room. A wall two rooms share is open if either room opens it.</p></section>${finishesHTML(r)}<section><h3>Doors, windows and archways <span>${(r.openings || []).length}</span></h3>${openings}${addOpening}</section><section><h3>Add a room beside it</h3>${add}</section><section><h3>Remove</h3><button data-action="room-delete" data-room="${r.id}" class="danger wide">${icon("trash-2")}<span>Delete ${e(r.name)}</span></button></section>`;
  }
  /**
   * Rooms → Finishes: what the selected room's floor, walls and ceiling are,
   * and any one wall finished differently from the rest. A wall between two
   * rooms is finished from each side separately.
   */
  function finishesHTML(r) {
    const surface = (part, label, f, surfaceKind, sameAs) => {
      const scope = `finish:${r.id}:${part}`,
        own = r.finishes?.[part];
      const options = `${sameAs ? `<option value="" ${own ? "" : "selected"}>${e(sameAs)}</option>` : ""}${SURFACE_KINDS[surfaceKind].map((k) => `<option value="${k}" ${(own || !sameAs) && f.kind === k ? "selected" : ""}>${e(FINISH_KINDS[k].label)}</option>`).join("")}`;
      return `<div class="finish-row"><label class="setting-label">${label}<select data-field="kind" data-scope="${scope}" aria-label="${e(r.name)} ${label.toLowerCase()} finish">${options}</select></label>${!sameAs || own ? colorField("Colour", "color", scope, f.color, `${r.name} ${label.toLowerCase()} colour`) : ""}</div>`;
    };
    const sides = SIDES.filter((s) => !(r.open || []).includes(s))
      .map((s) => surface(s, `${SIDE_NAMES[s]} wall`, wallFinish(r, s, p.booth.color), "walls", "Same as the walls"))
      .join("");
    return `<section><h3>Finishes</h3>${surface("floor", "Floor", floorFinish(r), "floor")}${surface("walls", "Walls", wallFinish(r, "walls", p.booth.color), "walls")}${surface("ceiling", "Ceiling", ceilingFinish(r), "ceiling")}<details class="finish-sides" ${SIDES.some((s) => r.finishes?.[s]) ? "open" : ""}><summary>One wall differently</summary>${sides}</details><p class="muted">Each face of a wall between two rooms is finished from its own room. The ceiling shows from inside a room and in Walk; from above the house stays open. Drop photographs of real surfaces into <code>assets/textures/finish-wood</code> and the like and they replace the drawn patterns.</p></section>`;
  }
  /** 150 → 12′ 6″. The room list reads in feet, as a house is talked about. */
  function fmtFeet(inches) {
    const ft = Math.floor(inches / 12),
      rest = Math.round(inches - ft * 12);
    return rest ? `${ft}′ ${rest}″` : `${ft}′`;
  }
  /**
   * After any room edit: every room inside its limits, the footprint grown or
   * shrunk to hold them, and each work pulled back onto its wall — a wall can
  ...
  function render() {
    ...
    document.querySelector("#scene-title").textContent =
      p.mode === "photo"
        ...
        : `${homeRooms(p).length} room${homeRooms(p).length === 1 ? "" : "s"} · ${fmtFeet(p.booth.width)} × ${fmtFeet(p.booth.depth)} floor`;
  ...
  /**
   * Start a new home: from the starter floor, from an empty floor, or from
   * one of the user's saved templates, with a backup of the current project
   * downloaded first unless they say otherwise.
   */
  function openNewHome() {
    const d = document.querySelector("#dialog");
    const templates = readTemplates();
    document.querySelector("#dialog-content").innerHTML =
      `<div class="panel-heading"><h2>Start a new home</h2>${btn("qs-cancel", "Close", "x", "icon-only")}</div><form class="quick-start" id="quick-start" onsubmit="return false"><label class="setting-label">Name<input id="qs-name" maxlength="120" placeholder="e.g. Maple Street" aria-label="Home name"/></label><label class="setting-label">Start from<select id="qs-template" aria-label="Start from"><option value="">The starter floor · four rooms</option><option value="empty">An empty floor</option>${templates.map((t) => `<option value="${e(t.id)}">My template · ${e(t.label)}</option>`).join("")}</select></label><label class="check-field"><input type="checkbox" id="qs-backup" checked/>Download a backup of the current home first</label><p class="muted">Everything is a starting point: rooms, furniture, finishes and lighting all stay editable. Your artwork library is not carried over.</p><div class="button-row"><button type="button" data-action="qs-cancel">Cancel</button><button type="button" class="primary" data-action="qs-go">Start this home</button></div>${templates.length ? `<p class="muted">Remove a saved template: ${templates.map((t) => `<button type="button" class="text-button" data-action="qs-forget-${e(t.id)}">${e(t.label)} ×</button>`).join(" ")}</p>` : ""}</form>`;
    refreshIcons();
    d.showModal();
  }
  function runNewHome() {
    const d = document.querySelector("#dialog");
    const name = (document.querySelector("#qs-name")?.value || "").trim().slice(0, 120);
    const templateId = document.querySelector("#qs-template")?.value || "";
    const template = templateId && readTemplates().find((t) => t.id === templateId);
    let next;
    if (template) next = fromTemplate(template, name);
    else {
      next = homeProject();
      if (templateId === "empty") {
        next.booth.rooms = [];
        Object.assign(next.booth, houseExtent([]));
      }
    ...
    try {
      ...
      toast("That template no longer opens in this version. Start from the starter floor instead.", true);
      return;
    }
  ...
  function addPhotoLayer(a) {
    if (!p.photo.asset) {
      toast("Upload a room photo first.", true);
      return;
    }
    if (a.kind === "sign" || a.kind === "label") {
  ...
  const actions = {
    ...
    "save-template": () => {
      ...
      if (writeTemplates(next))
        toast(`Saved “${t.label}” as a template${next.length > MAX_TEMPLATES ? `; the oldest of ${MAX_TEMPLATES} was dropped` : ""}. Start a new home offers it next time. Artwork and images are not included.`);
      else toast("This browser would not store the template. Download a project backup instead.", true);
    },
    "new-project": () => openNewHome(),
    "copy-project": () => {
      backup();
      mutate(() => {
        p.id = uid();
    ...
    help: () => {
      const d = document.querySelector("#dialog");
      document.querySelector("#dialog-content").innerHTML =
        `<div class="panel-heading"><h2>Welcome to Home Layout</h2>${btn("close-help", "Close", "x", "icon-only")}</div><p>Plan your home to measure, then hang your own art on its walls.</p><ol><li><strong>Rooms:</strong> the Rooms tab holds your floor plan. Add a room beside another and they share one wall; size each in inches; untick a wall to open two rooms into each other.</li><li><strong>Doors, windows and archways:</strong> added per room and placed along a wall. A door in a shared wall is one opening, seen from both rooms.</li><li><strong>Art:</strong> upload your originals in the library, then hang them on any room wall and give them their real size. Arrow keys nudge the selected work.</li><li><strong>Furniture and stairs:</strong> in the Walls tool. Double-click a piece in the view to pick it up and drag it.</li><li><strong>Views:</strong> Perspective looks into the house from above; Plan is the floor plan with door swings; Walk puts you inside at eye height.</li><li><strong>Keep your work:</strong> everything saves on this device. Download a project backup to move it or keep it safe.</li></ol>`;
      refreshIcons();
      d.showModal();
    },
    ...
    "add-panel": () =>
      mutate(() => {
        ...
        toast("Free-standing wall added at the centre of the floor. Drag it in the viewport or use the sliders below.");
      }),
    "open-walls": () => {
      tab = "walls";
      renderInspector();
    ...
    "add-pedestal": () =>
      mutate(() => {
        ...
        const kind = furnitureChoice,
          { label, limits, room: _group, layer, booth, wall, ...size } = FURNITURE[kind],
          // In a house, the room open in the Rooms tab.
          room = findRoom(p, selectedRoom) || homeRooms(p)[0],
          same = list.filter((x) => furnitureKind(x) === kind).length;
        // In a room, a piece that stands against a wall goes against its
        // north wall, its back to the wall and a second one beside the
        // first; anything else in the middle of the room. With no rooms (the
        // booth fixture), near the entrance on the right, as it always was.
        const along = room && wall ? Math.min(room.width / 2 - size.width / 2, -room.width / 2 + size.width / 2 + 12 + same * (size.width + 6)) : 0;
        list.push({
          id: uid(),
          name: (kind === "pedestal" ? "Pedestal" : label) + " " + (same + 1),
          ...size,
          ...(kind === "pedestal" ? {} : { kind }),
          // To the half inch a room's own edges fall on, so flush is flush.
          x: room ? Math.round((room.x + along) * 2) / 2 : Math.round(p.booth.width / 2 - size.width / 2 - 6),
          z: room ? Math.round((wall ? room.z - room.depth / 2 + size.depth / 2 : room.z) * 2) / 2 : Math.round(p.booth.depth / 2 - size.depth / 2 - 18),
          rotation: 0,
        });
        list[list.length - 1] = constrainPedestal(p, list[list.length - 1]);
        selectedPedestal = list[list.length - 1].id;
        selectedPanel = null;
        toast(`${label} added${room ? " to the " + room.name.toLowerCase() : ""}. Double-click it in the viewport to pick it up, or use the sliders.`);
      }),
    ...
    "room-first": () => {
      const r = newRoom({ type: "living", name: "Living room" });
      mutate(() => {
        p.booth.rooms = [r];
        selectedRoom = r.id;
        settleRooms();
    ...
    "room-starter": () =>
      confirmAction("Start from the starter floor?", "Every room is replaced by a living room, kitchen, bedroom and bathroom. Art on the old rooms' walls is taken down. Undo brings it all back.", () => {
        mutate(() => {
          const home = homeProject();
          p.booth.rooms = home.booth.rooms;
          selectedRoom = p.booth.rooms[0].id;
    ...
    "room-add": (b) => {
      const from = findRoom(p, selectedRoom);
      if (!from || homeRooms(p).length >= MAX_ROOMS) return;
      const type = newRoomType;
      const size = { living: [192, 168], kitchen: [144, 144], dining: [144, 144], bedroom: [144, 144], bath: [96, 108], office: [120, 120], hall: [48, 144], closet: [48, 48], laundry: [72, 96] }[type] || [120, 120];
      const side = b.dataset.side;
      const placed = roomBeside(from, side, { width: size[0], depth: size[1] });
      const r = newRoom({ type, name: ROOM_TYPES[type], height: from.height, ...placed });
      // A door in the shared wall, so the new room can be walked into.
      const shared = { n: "s", s: "n", e: "w", w: "e" }[side];
      const length = roomEdge(r, shared).length;
      r.openings = [newOpening(type === "hall" ? "arch" : "door", { side: shared, x: Math.max(0, Math.min(length - 36, 12)), width: Math.min(type === "hall" ? 40 : 32, length) })];
      mutate(() => {
        p.booth.rooms = [...homeRooms(p), r];
        selectedRoom = r.id;
        settleRooms();
      });
      toast(`${r.name} added ${SIDE_NAMES[side].toLowerCase()} of ${from.name}, with a door between them.`);
    },
    "room-delete": (b) => {
    ...
    "opening-add": (b) => {
      ...
      const length = roomEdge(r, "s").length,
        width = OPENING_KINDS[kind].width;
      mutate(() => {
        r.openings = [...(r.openings || []), newOpening(kind, { side: "s", x: Math.max(0, (length - width) / 2) })];
        settleRooms();
      });
    },
    "opening-delete": (b) => {
      ...
      if (!r) return;
      mutate(() => {
        r.openings = (r.openings || []).filter((o) => o.id !== b.dataset.opening);
        settleRooms();
      });
    },
    ...
    "export-glb": async () => {
      ...
      busy = true;
      try {
        toast("Writing the home as .glb…");
        const blob = await scene.exportGLB();
        download(blob, safeName() + ".glb");
        toast(`Home exported as .glb · ${(blob.size / 1048576).toFixed(1)} MB. Opens in SketchUp (with the glTF importer), Blender and AR viewers.`);
      } catch (err) {
    ...
    "view-save": () => {
      ...
      toast(`Saved as “${v.name}”. Rename it in the list; the View menu under the viewport goes back to it.`);
    },
    "views-export": async () => {
      if (busy || !scene) return;
      const views = p.views || [];
    ...
    if (b.dataset.action) {
      try {
        ...
        if (feature && !allowed(feature)) {
          toast(`${PRO_FEATURES[feature]} is part of Home Layout Pro. Layout → Project → Plan switches to Pro.`, true);
          return;
        }
        if (action.startsWith("view-go-")) goToView(action.slice(8));
  ...
  function placeCopyAt(sourceKey, ev) {
    ...
    if (!placed) {
      toast("Drop it directly onto a wall, inside or outside.", true);
      return false;
    }
    mutate(() => {
      p.art.push(placed);
  ...
  document.querySelector("#model-input").onchange = async (ev) => {
    ...
    ev.target.value = "";
    if (!file) return;
    if (!allowed("glb")) return toast("3D model import is part of Home Layout Pro.", true);
    try {
      ...
      const assetId = uid();
      // In the room open in the Rooms tab, like a piece from the furniture
      // list; the middle of the floor when there are no rooms.
      const room = findRoom(p, selectedRoom) || homeRooms(p)[0];
      const model = { id: uid(), asset: assetId, name: file.name.replace(/\.glb$/i, "").slice(0, 120), height: 36, x: room ? Math.round(room.x) : 0, z: room ? Math.round(room.z) : 0, rotation: 0 };
      mutate(() => {
        p.assets[assetId] = { name: file.name, width: 1, height: 1, role: "model", data };
        p.booth.models = [...models, model];
        tab = "walls";
      });
      toast(`${model.name} stands in the ${room ? room.name.toLowerCase() : "middle of the floor"} at 36″. Type its real height in the Walls tool.`);
    } catch (err) {
      toast(err.message, true);
    }
  };
  document.querySelector("#underlay-input").onchange = async (ev) => {
    ...
    ev.target.value = "";
    if (!file) return;
    if (!allowed("underlay")) return toast("The floor plan underlay is part of Home Layout Pro.", true);
    try {
    ...
    document.querySelector("#"+input).onchange=async ev=>{
      ...
      try{
        ...
        if(key==="surroundAsset" && Math.abs(asset.width/asset.height-2)>.1)
          throw new Error("Choose a 2:1 full-sphere panorama for this 360° background. Ordinary room photos belong in Photo mode.");
        mutate(()=>{
          const id=uid();
  ...
  function fieldTarget(scope, art) {
    // A room finish, resolved into the room so a swatch can set its colour.
    if (scope?.startsWith("finish:")) {
      const [, roomId, part] = scope.split(":");
      const r = findRoom(p, roomId);
      if (!r) return null;
      const resolved = part === "floor" ? floorFinish(r) : part === "ceiling" ? ceilingFinish(r) : wallFinish(r, part, p.booth.color);
      r.finishes = { ...(r.finishes || {}), [part]: { ...resolved } };
      return r.finishes[part];
    }
    return (
        scope === "art"
          ? art
  ...
  document.addEventListener("change", (ev) => {
    ...
    if (el.id === "project-name") {
      mutate(() => (p.name = el.value.trim() || "Untitled home"));
      return;
    }
    ...
    if (el.id === "new-room-type") {
      newRoomType = el.value;
      return;
    }
    ...
    mutate(() => {
      ...
      if (String(scope).startsWith("room|")) {
        const r = findRoom(p, String(scope).slice(5));
        if (!r) return;
        if (key.startsWith("wall-")) {
          const side = key.slice(5),
            open = new Set(r.open || []);
          if (value) open.delete(side);
          else open.add(side);
          r.open = [...open];
        } else if (key === "type") {
          // A room still named for its old kind follows the new one.
          if (r.name === ROOM_TYPES[r.type || "other"]) r.name = ROOM_TYPES[value] || r.name;
          r.type = String(value);
        } else if (key === "name") r.name = String(value).slice(0, 120) || ROOM_TYPES[r.type || "other"];
        else if (key === "width" || key === "depth") Object.assign(r, resizeRoom(homeRooms(p), r, key, value));
        else r[key] = value;
        // Art that stood on a wall that is no longer there comes down.
        settleRooms();
        const keys = new Set(roomWalls(p).map((w) => w.key));
      ...
      // A finish: choosing a kind starts it at that kind's usual colour, and
      // "Same as the walls" takes a wall's own finish away again.
      if (String(scope).startsWith("finish:")) {
        const [, roomId, part] = String(scope).split(":");
        const r = findRoom(p, roomId);
        if (!r) return;
        const resolved = part === "floor" ? floorFinish(r) : part === "ceiling" ? ceilingFinish(r) : wallFinish(r, part, p.booth.color);
        const finishes = { ...(r.finishes || {}) };
        if (key === "kind" && !value) delete finishes[part];
        else if (key === "kind") finishes[part] = { kind: String(value), color: value === resolved.kind ? resolved.color : value === "paint" ? p.booth.color : KIND_COLORS[value] || resolved.color };
        else finishes[part] = { ...resolved, [key]: value };
        r.finishes = finishes;
        return;
      }
      if (String(scope).startsWith("opening|")) {
        ...
        const r = findRoom(p, roomId),
          o = r?.openings?.find((x) => x.id === openingId);
        if (!o) return;
        o[key] = value;
        if (key === "side") o.x = Math.max(0, Math.min(o.x, roomEdge(r, o.side).length - o.width));
        settleRooms();
        return;
      }
      if (scope === "corner") {
  ...
  async function upload(files, type) {
    ...
    try {
      ...
      toast(
        type === "photo"
          ? "Room photo ready. Select an uploaded artwork to add it."
          : type === "replace"
  ...
  const TAB_NAMES = { art: "Artwork", rooms: "Rooms", layout: "Layout", show: "Art show", walls: "Walls", lighting: "Lighting", video: "Video", export: "Export" };
...
boot().catch((err) => {
  console.error(err);
  document.querySelector("#app").innerHTML =
    "<div><h1>Home Layout could not start</h1><p>Please reload or try another browser. Existing saved data has not been removed.</p></div>";
});
