...
.filter(([id, asset]) => !excluded.has(id) && (asset.role === "artwork" || p.art.some((item) => item.asset === id)))
  ...
  function underlaySection() {
    ...
    return gated("underlay", `<section><h3>Floor plan underlay</h3>${body}</section>`, "Lay the venue's floor plan under your booth at its real scale, set by two clicks with the tape. Part of Home Layout Pro.");
  }
  /**
  ...
  function finishesSection(r, scope) {
    ...
    const sides = SIDES.map((side) => {
      ...
      return `<div class="finish-side"><label class="setting-label">${SIDE_NAMES[side]} wall<select data-field="finish-side-mode-${side}" data-scope="${scope}" aria-label="${SIDE_NAMES[side]} wall paint"><option value="room" ${own ? "" : "selected"}>Same as the room</option><option value="own" ${own ? "selected" : ""}>Its own colour</option></select></label>${own ? color("", "side-" + side, own, `${SIDE_NAMES[side]} wall colour`) : ""}</div>`;
    }).join("");
