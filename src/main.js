...
// under their own heading: they are still valid pieces, and a pedestal or a
// plinth is as much a home's as a show's.
const FURNITURE_GROUPS = [
  ["Living", ["sofa", "armchair", "coffee", "tv", "shelves", "rug"]],
  ["Bedroom", ["bed", "nightstand", "dresser"]],
  ["Dining and kitchen", ["dining", "chair", "stool", "cabinet", "wallcab", "counter"]],
  ["Work", ["desk"]],
  ["Structure", ["stairs", "box"]],
  ["Display", ["pedestal", "table6", "table8", "bin", "gridwall", "banner"]],
];
async function boot() {
  const icons = {
    Grid2x2: Grid2X2,
...
<div class="workspace"><aside class="library" id="library"></aside><main class="editor"><div class="toolbar"><div class="toolgroup">${btn("select", "Select", "mouse-pointer-2", "active")}${btn("move", "Move", "move")}${btn("snap", "Snap 1″", "grid-2x2", "active")}${btn("measure", "Measure", "ruler")}${btn("walk", "Walk", "footprints")}${btn("draw-box", "Box", "square")}${btn("draft", "Fast edit", "zap")}${btn("draft-lock", "Fast edit: follows the gesture", "lock", "draft-lock")}</div><div class="toolgroup">${btn("undo", "Undo", "undo-2", "icon-only")}${btn("redo", "Redo", "redo-2", "icon-only")}</div><div class="mode-switch"><button data-action="mode-3d">3D home</button><button data-action="mode-photo">Photo</button></div>${btn("export-tab", "Export", "download", "export-top")}</div><div class="viewport"><div id="scene"></div><div id="photo" hidden></div><div class="scene-label"><span class="eyebrow" id="mode-label">MEASURED WORKSPACE</span><strong id="scene-title"></strong><span id="scene-subtitle"></span></div><div id="photo-empty" hidden><div>${icon("image-plus")}<h2>Start with a photo of your room</h2><p>Add artwork and adjust its four corners to match the wall perspective.</p>${btn("upload-photo", "Upload room photo", "plus", "primary")}</div></div><div class="walk-pad" hidden><button data-walk="forward" aria-label="Step forward">▲</button><button data-walk="left" aria-label="Step left">◀</button><button data-walk="back" aria-label="Step back">▼</button><button data-walk="right" aria-label="Step right">▶</button><button data-action="walk" class="walk-exit" aria-label="Stop walking">Done</button></div><div class="viewport-bottom"><div class="view-switch" id="view-switch"><button data-view="perspective" class="active">Perspective</button><button data-view="back">Back</button><button data-view="left">Left</button><button data-view="right">Right</button><button data-view="plan">Plan</button></div><label class="saved-view-pick" hidden><span>View</span><select id="saved-view" aria-label="Go to a saved view"></select></label><div class="zoom-controls"><span class="zoom-label">Zoom</span>${btn("zoom-out", "Zoom out", "minus", "icon-only")}${btn("zoom-in", "Zoom in", "plus", "icon-only")}${btn("reset-view", "Reset view", "rotate-ccw", "icon-only")}</div></div></div><div class="statusbar"><span id="gesture-hint">Drag to orbit · scroll or +/− to zoom · right-drag to pan</span><label class="preview-quality" title="Preview quality: how many pixels the viewport draws for each one on screen. Exports are never affected."><span>Preview</span><select id="quality-quick" aria-label="Preview quality"></select><output id="quality-now"></output></label><span id="selection-status"></span></div></main><aside class="inspector"><button class="sheet-toggle" data-action="sheet-toggle" aria-label="Fold the panel away" title="Fold the panel away"><span>Fold</span></button><div class="inspector-tabs">${["art", "rooms", "layout", "walls", "lighting", "video", "export"].map((t, i) => `<button data-tab="${t}">${icon(["image", "house", "layout-panel-left", "columns-2", "lightbulb", "video", "download"][i])}<span>${["Artwork", "Rooms", "Layout", "Walls", "Lighting", "Video", "Export"][i]}</span></button>`).join("")}</div><div id="inspector-content"></div></aside></div><footer><span class="footer-brand">${icon("house")} HOME LAYOUT <small>Preview</small><small id="build-stamp" title="Version ${BUILD.version} · built ${BUILD.time} · commit ${BUILD.commit}">v${BUILD.version} · ${BUILD.short} UTC · ${BUILD.commit}</small></span><span>Your images. Your space. Your arrangement.</span><span id="network">Local workspace</span></footer><input type="file" id="art-input" accept="image/jpeg,image/png" multiple hidden/><input type="file" id="replace-input" accept="image/jpeg,image/png" hidden/><input type="file" id="photo-input" accept="image/jpeg,image/png" hidden/><input type="file" id="surround-input" accept="image/jpeg,image/png" hidden/><input type="file" id="ground-input" accept="image/jpeg,image/png" hidden/><input type="file" id="underlay-input" accept="image/jpeg,image/png" hidden/><input type="file" id="model-input" accept=".glb,model/gltf-binary" hidden/><input type="file" id="backup-input" accept=".json,.booth" hidden/><div id="toast" role="status"></div><dialog id="dialog"><div id="dialog-content"></div></dialog><dialog id="image-editor"><div id="image-editor-content"></div></dialog><dialog id="timeline-dialog" class="timeline-dialog"><div id="timeline-content"></div></dialog>`;
  ...
  if (scene) {
    ...
    // Clicking a room's floor opens Rooms on that room, the way clicking a
    // work opens Artwork on it.
    scene.onSelectRoom = (id) => {
      if (!findRoom(p, id)) return;
      selectedRoom = id;
      tab = "rooms";
      renderSelection();
    };
  ...
  function refreshScene() {
    ...
    if (refitOwed && scene && !scene.walking && p.mode === "3d") scene.setView(scene.view);
    refitOwed = false;
    syncClearance();
    photo.update(p, photoSelected);
  }
  ...
  function panelFields() {
    ...
    return `<section><h3>Free-standing walls</h3><p class="muted">Interior panels you can stand anywhere in the home and hang art on either side. Position is measured in inches from the centre of the floor: X is right, Z is toward the entrance. They do not change the floor.</p><p class="muted">Click a free-standing wall in the home to select it, then drag it across the floor or use the sliders. Snap keeps a drag on whole inches.</p>${panels.map((panel, i) => {
  ...
  function pedestalFields() {
    const list = boothPedestals(p);
    return `<section><h3>Furniture</h3><p class="muted">Beds, sofas and chairs, tables and desks, kitchen cabinets, shelves, a dresser, rugs, a screen and stairs, each at a common size to type over. A new piece lands in the middle of the room open in the Rooms tab, facing south; its back is its north side. Double-click one in the house to pick it up, then drag it across the floor or use the sliders. Position is inches from the centre of the floor: X is right, Z is toward the entrance. Plan view shows the clear floor around the one selected.</p>${list.map((ped, i) => {
    ...
    }).join("")}${list.length < MAX_PEDESTALS ? `<label class="setting-label">Add to the floor<select id="furniture-kind" aria-label="Furniture to add">${FURNITURE_GROUPS.map(([group, kinds]) => `<optgroup label="${e(group)}">${kinds.filter((k) => FURNITURE[k] && (k !== "box" || allowed("box"))).map((k) => `<option value="${k}" ${k === furnitureChoice ? "selected" : ""}>${e(FURNITURE[k].label)} · ${FURNITURE[k].width}×${FURNITURE[k].depth}″</option>`).join("")}</optgroup>`).join("")}</select></label>${btn("add-pedestal", "Add " + FURNITURE[furnitureChoice].label.toLowerCase(), "plus", "wide")}` : `<p class="muted">${MAX_PEDESTALS} pieces is the limit.</p>`}</section>`;
  }
  /**
...
.filter(([id, asset]) => !excluded.has(id) && (asset.role === "artwork" || p.art.some((item) => item.asset === id)))
  ...
  function rowSection() {
    if (!allowed("row"))
      return proLock("row", "Booths either side of this one, spaces in the aisle and artwork hung in each — the whole row, measured. Part of Home Layout Pro.");
    const row = normalizeRow(p.booth);
  ...
  function viewsSection() {
    ...
    const rows = views
      ...
      .join("");
    return `<section><h3>Saved views <span>${views.length} / ${MAX_VIEWS}</span></h3><p class="muted">Frame the home the way you want it seen, then save the view. Each one is a click away here and in the View menu under the viewport, and they are saved with the project.</p>${rows}${views.length < MAX_VIEWS ? btn("view-save", "Save this view", "bookmark", "wide") : `<p class="muted">${MAX_VIEWS} views is the limit.</p>`}${views.length ? btn("views-export", `Export all ${views.length} as PNG`, "download", "wide") : ""}</section>`;
  }
  /**
  ...
  function tagsSection() {
    return `<section><h3>Tags <span>show or hide</span></h3><p class="muted">Hide a whole group at once — in the viewport and in exports. Nothing is deleted, and reopening the project shows everything again.</p><div class="tag-list">${Object.entries(TAGS)
  ...
  function modelsSection() {
    ...
    const rows = models
      ...
      .join("");
    return gated("glb", `<section><h3>3D models <span>${models.length} / ${MAX_MODELS}</span></h3><p class="muted">Bring in a .glb from SketchUp, Blender or a 3D scan — a sculpture, a custom display — and stand it on the floor. It is scaled to the height you type, so its own units do not matter. Saved in the project and its backups.</p>${rows}${models.length < MAX_MODELS ? btn("upload-model", "Import .glb model", "upload", "wide") : `<p class="muted">${MAX_MODELS} models is the limit.</p>`}</section>`, "Bring in .glb models from SketchUp, Blender or a scan, and export the home as .glb. Part of Home Layout Pro.");
  }
  /**
  ...
  function underlaySection() {
    ...
    const body = !u
      ? `<p class="muted">Put the show's floor plan under your home: a JPG or PNG of it (a screenshot of a PDF plan is fine). It lies on the floor, half see-through, and never reaches an export.</p>${btn("upload-underlay", "Add floor plan image", "map", "wide")}`
    ...
    return gated("underlay", `<section><h3>Floor plan underlay</h3>${body}</section>`, "Lay the venue's floor plan under your home at its real scale, set by two clicks with the tape. Part of Home Layout Pro.");
  }
  /**
  ...
  function clearanceSection() {
    ...
    const rows = issues
      ...
      .join("");
    return gated("clearance", `<section><h3>Clearance checks <span>${issues.length ? issues.length + " to look at" : "all clear"}</span></h3><p class="muted">Floor pieces standing in each other or outside the booth, works hung over each other, and gaps narrower than the ${ACCESSIBLE}″ a wheelchair needs. Plan view draws the tight gaps in red. Pieces pushed right up against something are taken as meant.</p>${issues.length ? `<ul class="clearance-list">${rows}</ul>` : `<p class="ok-note">Nothing is in the way.</p>`}</section>`, "Warnings for pieces standing in each other or outside the booth, and gaps too narrow for a wheelchair. Part of Home Layout Pro.");
  }
   ...
   */
  /**
   * A room's finishes: its paint, a feature wall, its floor and its ceiling.
   * Colours are plain colour inputs plus named paint chips, each chip a
   * `data-finish` button so one click paints; "Same as the room" clears a
   * side's own colour. Everything writes through setFinish, so a finish that
   * goes back to every default leaves no record behind.
   */
  function finishesSection(r, scope) {
    const fin = roomFinish(r),
      walls = sideColor(p, { ...r, finish: { walls: fin.walls } }, "n");
    const chips = (key, current) =>
      `<div class="swatches">${Object.entries(PAINTS).map(([c, name]) => `<button data-finish="${key}" data-room="${r.id}" data-value="${c}" style="background:${c}" title="${e(name)}" aria-label="${e(name)} ${key === "ceiling" ? "ceiling" : "paint"}"${c === current ? ' class="active"' : ""}></button>`).join("")}</div>`;
    const color = (label, key, value, aria) =>
      `<label class="color-field">${label}<input type="color" data-field="finish-${key}" data-scope="${scope}" aria-label="${e(aria)}" value="${e(value)}"/></label>`;
    const sides = SIDES.map((side) => {
      const own = fin.sides?.[side];
  ...
  function settleRooms() {
    ...
    const before = { width: p.booth.width, depth: p.booth.depth };
    Object.assign(p.booth, houseExtent(p.booth.rooms));
    // A house that outgrew the view is framed again on the next redraw, so a
    // new room is never added out of sight. Shrinking leaves the view alone.
    if (p.booth.width > before.width || p.booth.depth > before.depth) refitOwed = true;
  ...
  function inspectorHTML() {
    ...
    if (tab === "walls") {
      html = p.mode === "photo"
        ? `<div class="panel-heading"><h2>Walls and pedestals</h2>${icon("columns-2")}</div><div class="empty-inspector"><p>Free-standing walls and pedestals are measured objects in the 3D home. Switch to the 3D home to place them.</p></div>`
        : `<div class="panel-heading"><h2>Walls and pedestals</h2>${icon("columns-2")}</div><p class="muted">Everything that stands on the floor: interior display walls you can hang art on, and pedestals you cannot. Both are placed in inches and both can be dragged.</p>${panelFields()}${pedestalFields()}${modelsSection()}`;
    }
    if (tab === "lighting") {
  ...
  const actions = {
    ...
    "add-pedestal": () =>
      mutate(() => {
        ...
        const kind = furnitureChoice,
          { label, limits, ...size } = FURNITURE[kind],
          // In a house, the middle of the room open in the Rooms tab.
        ...
        list.push({
          ...
          ...size,
          ...(kind === "pedestal" ? {} : { kind }),
          x: room ? Math.round(room.x) : Math.round(p.booth.width / 2 - size.width / 2 - 6),
          z: room ? Math.round(room.z) : Math.round(p.booth.depth / 2 - size.depth / 2 - 18),
          rotation: 0,
        });
        list[list.length - 1] = constrainPedestal(p, list[list.length - 1]);
        selectedPedestal = list[list.length - 1].id;
        selectedPanel = null;
        toast(`${label} added${room ? " in the " + room.name.toLowerCase() : ""}. Double-click it to pick it up, or use the sliders.`);
      }),
    ...
    "room-add": (b) => {
      ...
      const r = newRoom({ type, name: ROOM_TYPES[type], height: from.height, ...placed, ...(starterFinish(type) ? { finish: starterFinish(type) } : {}) });
  ...
  document.addEventListener("click", (ev) => {
    ...
    if (b.dataset.finish && b.dataset.room) {
      const r = findRoom(p, b.dataset.room);
      if (r)
        mutate(() => {
          r.finish = setFinish(r, b.dataset.finish, b.dataset.value);
          if (!r.finish) delete r.finish;
        });
    }
  ...
  function placeCopyAt(sourceKey, ev) {
    ...
    if (!placed) {
      toast("Drop directly on a room wall or a free-standing wall.", true);
      return false;
    }
    mutate(() => {
      p.art.push(placed);
  ...
  document.querySelector("#model-input").onchange = async (ev) => {
    ...
    try {
      ...
      toast(`${model.name} stands in the home at 36″. Type its real height in the Walls tool.`);
    } catch (err) {
      toast(err.message, true);
    }
  };
  document.querySelector("#underlay-input").onchange = async (ev) => {
    ...
    try {
      ...
      scene?.setView("plan");
      document.querySelectorAll("[data-view]").forEach((b) => b.classList.toggle("active", b.dataset.view === "plan"));
      toast("Floor plan laid under the home. Measure something you know with the tape (T), then Scale plan.");
    } catch (err) {
      toast(err.message, true);
    }
  };
  ...
  document.addEventListener("change", (ev) => {
    ...
    mutate(() => {
      ...
      if (String(scope).startsWith("room|")) {
        ...
        if (!r) return;
        if (key.startsWith("finish-side-mode-")) {
          const side = key.slice(17);
          r.finish = setFinish(r, "side-" + side, value === "own" ? sideColor(p, r, side) : undefined);
          if (!r.finish) delete r.finish;
          return;
        }
        if (key.startsWith("finish-")) {
          r.finish = setFinish(r, key.slice(7), value);
          if (!r.finish) delete r.finish;
          return;
        }
